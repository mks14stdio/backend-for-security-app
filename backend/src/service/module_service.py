...
class ModuleService:
    ...
    async def add_module(self, data: ModuleCreate):
        try:
            module = await self.repository.add_one(
                ModuleBase(title=data.title).model_dump()
            )
    ...
    async def update_module_item(self, data: ModuleItemUpdate, item_id: int):
        try:
            module_item = await self.item_repository.update_one(
                item_id, data.model_dump()
            )
            await self.session.commit()
    ...
    async def delete_module_item(self, id: int, item_id: int):
        try:
            ...
            for item in item_to_update:
                await self.item_repository.update_one(
                    item.id, {"order_index": item.order_index - 1}
                )
    ...
    async def add_module_item(self, data: ModuleItemCreate, module_id: int):
        try:
            ...
            module_item = await self.item_repository.add_one(
                ModuleItemBase(
                    module_id=module_id,
                    title=data.title,
                    article_id=data.article_id,
                    order_index=len_module,
                ).model_dump()
            )
            await self.session.commit()
            return ModuleItemRead.model_validate(module_item)
